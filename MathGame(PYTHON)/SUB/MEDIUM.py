...
def medium():
    ...
    NAME=input("\nType Your Name:")
    print("GOODLUCK",NAME)
    print("The difficulty of the game is MEDIUM")
    #Allows the user to enter the number of questions they want to answer.
    Q=int(input("\nEnter The Number Of Questions You Wish To Answer"))
    questions=[]
    for i in range(1,Q+1):
        print("\n")
        s=random.choice(['+','-'])
        q=str(random.randrange(50)) + s + str(random.randrange(50))
        USERans=int(input(q+ "="))
    ...
    Insert=("INSERT INTO custom_game(Name, CORRECT_questions, TOTAL_questions, PERCENTAGE,LEVEL) VALUES (%s,%s,%s,%s,'MEDIUM')")
    val=NAME, C, Q, P
